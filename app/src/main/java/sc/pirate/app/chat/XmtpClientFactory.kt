...
private const val TAG = "XmtpClientFactory"
private const val PREFS_NAME = "xmtp_prefs"
private const val KEY_DB_KEY = "db_encryption_key"

internal fun getOrCreateXmtpDbKey(appContext: Context, identity: String): ByteArray {
    val prefs = appContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
    val storageKey = "$KEY_DB_KEY:${identity.lowercase()}"
    prefs.getString(storageKey, null)?.let { return hexToBytes(it) }
...
internal suspend fun createXmtpClientWithDbRecovery(
    ...
    return try {
        ...
        if (!isDbKeyOrSaltMismatch(error)) throw error
        Log.w(TAG, "XMTP DB key/salt mismatch detected; resetting local XMTP DB and retrying once", error)
